...
const JWT_SECRET = process.env.JWT_SECRET || 'prode2026-secret-changeme-in-production';

const dbDir = path.join(__dirname, 'db');
if (!fs.existsSync(dbDir)) fs.mkdirSync(dbDir);
const db = new Database(path.join(dbDir, 'prode.db'));
...
function calcScore(pred, result) {
  ...
  if (pred.h1 !== null && pred.a1 !== null) {
    ...
    if (predSign === resSign) { pts += 2; if (pred.h1 === result.h1 && pred.a1 === result.a1) pts += 2; }
  }
  if (pred.ya !== null && pred.yb !== null && result.ya !== null) {
    ...
    if (predSign === resSign) { pts += 1; if (pred.ya === result.ya && pred.yb === result.yb) pts += 1; }
  }
  if (pred.ca !== null && pred.cb !== null && result.ca !== null) {
    ...
    if (predSign === resSign) { pts += 1; if (pred.ca === result.ca && pred.cb === result.cb) pts += 1; }
  }
  return pts;
}
...
app.post('/api/register', (req, res) => {
  ...
  if (password.length < 4) return res.status(400).json({ error: 'Contraseña muy corta (mín 4 chars)' });
...
app.post('/api/predictions/:matchId', authMiddleware, (req, res) => {
  ...
  db.prepare(`
    INSERT INTO predictions (user_id, match_id, h1, a1, ya, yb, ca, cb, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
...
app.post('/api/admin/result/:matchId', adminMiddleware, (req, res) => {
  ...
  const { h1, a1, ya, yb, ca, cb } = req.body;
  db.prepare(`
    INSERT INTO results (match_id, h1, a1, ya, yb, ca, cb, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
